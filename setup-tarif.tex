...
\N
Скопировать файл "АТС архив.mdb" в домашний каталог сервера базы данных.
...
Сначала заходим на MGMT01, с него на MGMT02. С диска S на MGMT02 копируем правой кнопкой мыши.
С MGMT02 на сервер копируем через WinSCP.
...
\noindent
Теперь (учитывая что установлен web-сервер -- см.\ следующий раздел) на MGMT02
открываем в браузере \hfil\break
"https://192.168.40.34/otdel" \hfil\break и сохраняем файлы детализации в папку Загрузки.
Потом оттуда переносим на MGMT01 правой кнопкой мыши. А на MGMT01 раскладываем в папку
"Детализация ИТ и связи".
...
\smallskip
\noindent
Для каждой станции, с которой необходимо собирать тарификацию, создаётся своя программа. Кроме
того, что эти программы заносят информацию в базу данных, некоторые из них ещё и конвертируют
данные из формата каждой конкретной станции в общий формат (если используется программа сбора
тарификации, поставляемая производителем), перед тем как сохранить их в базу. Благодаря этому
система тарификации может быть адаптирована для работы с разными типами телефонных станций, при
этом для добавления новой станции нужно лишь создать соответствующую программу.
\bigskip
...
по сети: \par
\indent {\tt //192.168.40.141/D /media/upats/ cifs  username=<user>,password=<pass>,uid=user,port=139 0 0} \hfil\break
...
\noindent
Настройки для гудвина, миникома и корала см.\ в документации к программам {\tt gud},
{\tt minicom-zd4}, {\tt coral-zd26}, {\tt coral-zd5} и {\tt coral-w}.
\bigskip
...
\noindent $\diamond$ upats = {\tt SMPSpider+SMPCallBuilder+call\_build.pl} на 192.168.40.141 \par % user 1F5@
\noindent $\diamond$ goodwin = {\tt googwin-log.sh} в кроне на 192.168.40.199 \par
\noindent $\diamond$ Coral w = {\tt cdr-coral-w} в кроне на 192.168.40.53 \par % user 1
\noindent $\diamond$ Coral зд.5 = {\tt cdr-coral-zd5} в кроне на 192.168.40.54 \par % user std012
\noindent $\diamond$ minicom зд.4 = {\tt DX7term+DX7termRestart.pl} на 192.168.40.51 % удалённый рабочий стол user 1
